...
int main(int argc, char *argv[])
 {
	...
	sprintf(iperfshift4cmd, "iperf -c %s -t %s -f m -P 10 |grep bits/sec >> shift4.txt", argv[2], argv[3]);
	

	char iperflia4cmd[100] = "";
	sprintf(iperflia4cmd, "iperf -c %s -t %s -f m -P 10 |grep bits/sec >> lia4.txt", argv[2], argv[3]);

        char iperfshift2cmd[100] = "";
        sprintf(iperfshift2cmd, "iperf -c %s -t %s -f m -P 10 |grep bits/sec >> shift2.txt", argv[2], argv[3]);

	char iperflia2cmd[100] = "";
        sprintf(iperflia2cmd, "iperf -c %s -t %s -f m -P 10 |grep bits/sec >> lia2.txt", argv[2], argv[3]);


        char iperftcp1cmd[100] = "";
        sprintf(iperftcp1cmd, "iperf -c %s -t %s -f m -P 10 |grep bits/sec >> tcp1.txt", argv[2], argv[3]);

        char iperfdc1cmd[100] = "";
        sprintf(iperfdc1cmd, "iperf -c %s -t %s -f m -P 10 |grep bits/sec >> dc1.txt", argv[2], argv[3]);




	char scpshift4cmd[100] = "";
	sprintf(scpshift4cmd, "scp -i infocom_pair.pem shift4.txt ubuntu@%s:~/iperfrs/shift4-%s-%s.txt",argv[4], argv[1], time_stamp() );

	char scplia4cmd[100] = "";
        sprintf(scplia4cmd, "scp -i infocom_pair.pem lia4.txt ubuntu@%s:~/iperfrs/lia4-%s-%s.txt",argv[4], argv[1], time_stamp() );


	char scpshift2cmd[100] = "";
        sprintf(scpshift2cmd, "scp -i infocom_pair.pem shift2.txt ubuntu@%s:~/iperfrs/shift2-%s-%s.txt",argv[4], argv[1], time_stamp() );

	char scplia2cmd[100] = "";
        sprintf(scplia2cmd, "scp -i infocom_pair.pem lia2.txt ubuntu@%s:~/iperfrs/lia2-%s-%s.txt",argv[4], argv[1], time_stamp() );

	char scptcp1cmd[100] = "";
        sprintf(scptcp1cmd, "scp -i infocom_pair.pem tcp1.txt ubuntu@%s:~/iperfrs/tcp1-%s-%s.txt",argv[4], argv[1], time_stamp() );

	char scpdc1cmd[100] = "";
        sprintf(scpdc1cmd, "scp -i infocom_pair.pem dc1.txt ubuntu@%s:~/iperfrs/dc1-%s-%s.txt",argv[4], argv[1], time_stamp() );
	...
	//if(countline(filepath)==7){
		...
		int send=0;
		if( access( "shift4.txt", F_OK) == -1) { send=1; }
		if( access( "lia4.txt", F_OK) == -1) { send=1; }
		if( access( "shift2.txt", F_OK) == -1) { send=1; }
		if( access( "lia2.txt", F_OK) == -1) { send=1; }
		if( access( "tcp1.txt", F_OK) == -1) { send=1; }
		if( access( "dc1.txt", F_OK) == -1) { send=1; }
